const DashboardPage = {
  render() {
    ...
    const ongoing  = companies.filter(c => !c.finalResult);
    const passed   = companies.filter(c => c.finalResult === 'IS参加決定' || c.finalResult === '内定');
    const rejected = companies.filter(c => c.finalResult === 'お祈り');
    const today = new Date(); today.setHours(0,0,0,0);
    const in14  = new Date(today.getTime() + 14 * 86400000);

    const upcoming = companies
      .filter(c => c.scheduleDate && !c.finalResult)
      .map(c => ({ ...c, _d: new Date(c.scheduleDate) }))
      .filter(c => c._d >= today && c._d <= in14)
      .sort((a,b) => a._d - b._d);
    ...
    const recentCompanies = [...companies].sort((a,b) => new Date(b.updatedAt||b.createdAt) - new Date(a.updatedAt||a.createdAt)).slice(0,6);
...
<div class="page-content">
  ...
  <div class="dashboard-grid">
    <div class="card">
      <div class="card-header"><h3 class="card-title">直近14日のスケジュール</h3></div>
      <div class="card-body">
        ${upcoming.length === 0
          ? '<p class="empty-msg">直近14日以内のスケジュールはありません</p>'
          : `<table class="table">
              <thead><tr><th>企業名</th><th>Tier</th><th>ステージ</th><th>日程</th><th>残り</th></tr></thead>
              <tbody>
                ${upcoming.map(c => {
                  const d = daysUntil(c.scheduleDate);
                  const cls = d <= 3 ? 'text-danger fw-bold' : d <= 7 ? 'text-warning fw-bold' : '';
                  return `<tr>
                    <td><b>${esc(c.name)}</b></td>
                    <td>${renderTierBadge(c.tier)}</td>
                    <td>${renderStageBadge(c.currentStage)}</td>
                    <td>${formatDate(c.scheduleDate)}</td>
  ...
  <div class="card mt-4">
    ...
    <div class="card-body p0">
      <table class="table table-hover">
        <thead><tr><th>企業名</th><th>業界</th><th>Tier</th><th>選考状況</th></tr></thead>
        <tbody>
          ${recentCompanies.map(c => `<tr>
            <td><b>${esc(c.name)}</b></td>
